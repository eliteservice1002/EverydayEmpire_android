...
import app.everydayempire.tv.fragments.AdjustAudioFragment;
import app.everydayempire.tv.utils.TempUtil;
import app.everydayempire.tv.workers.DelayAudioWorker;
import app.everydayempire.tv.workers.MergeAudioVideoWorker2;
import app.everydayempire.tv.workers.MergeAudiosWorker;
import app.everydayempire.tv.workers.SplitAudioWorker;
...
public class AdjustAudioActivity extends AppCompatActivity implements AnalyticsListener {
    ...
    private void setupViews() {
        ...
        new TabLayoutMediator(tabs, pager, (tab, position) -> {
            tab.setText(position == 0 ? R.string.vocals_label : R.string.beat_label);
        }).attach();
        ...
        pager.registerOnPageChangeCallback(new ViewPager2.OnPageChangeCallback() {
            ...
            public void onPageSelected(int position) {
                ...
                if (position == 1 && !mModel.alertedS) {
                    mModel.alertedS = true;
                    new MaterialAlertDialogBuilder(AdjustAudioActivity.this)
                            .setTitle(R.string.alert_adjust_beat_title)
                            .setMessage(R.string.alert_adjust_beat_message)
                            .setPositiveButton(R.string.alert_adjust_beat_ok, (dialog, i) -> dialog.dismiss())
                            .show();
                }
            }
        });
        if (!mModel.alertedV) {
            mModel.alertedV = true;
            new MaterialAlertDialogBuilder(this)
                    .setTitle(R.string.alert_adjust_vocals_title)
                    .setMessage(R.string.alert_adjust_vocals_message)
                    .setPositiveButton(R.string.alert_adjust_vocals_ok, (dialog, i) -> dialog.dismiss())
                    .show();
        }
    }
    ...
    private void startAudioPlayerV() {
        ...
        long delay = Math.round(mModel.delayV.getValue()) - 1000;
        if (delay < 0) {
    ...
    private void startAudioPlayerS() {
        ...
        //noinspection ConstantConditions
        long delay = Math.round(mModel.delayS.getValue()) - 1000;
        if (delay < 0) {
    ...
    private void submitForAdjustment() {
        ...
        Data dataV = new Data.Builder()
                .putString(DelayAudioWorker.KEY_VIDEO, mVideo)
                .putString(DelayAudioWorker.KEY_AUDIO, mVideoAudio)
                .putLong(DelayAudioWorker.KEY_DELAY, delayV - 1000)
        ...
        if (TextUtils.isEmpty(mSong)) {
            ...
            Data dataS = new Data.Builder()
                    ...
                    .putLong(DelayAudioWorker.KEY_DELAY, delayS - 1000)
    ...
    public static class AdjustAudioActivityViewModel extends ViewModel {
        ...
        public MutableLiveData<Float> delayV = new MutableLiveData<>(1000f);
        public MutableLiveData<Float> delayS = new MutableLiveData<>(1000f);

        public MutableLiveData<Float> volumeV = new MutableLiveData<>(100f);
        public MutableLiveData<Float> volumeS = new MutableLiveData<>(0f);

        public boolean alertedV = false;
        public boolean alertedS = false;
